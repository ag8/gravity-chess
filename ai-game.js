...
function whiteMove() {
    function randomWhitePiece() {
        let chosenPiece = null;

        while (chosenPiece == null || chosenPiece.color !== 0) {
            chosenPiece = gamePieces[Math.floor(Math.random() * gamePieces.length)];
        }
    ...
    // Get a random piece that has some legal moves
    ...
    let [moveRow, moveCol] = legalMoves[Math.floor(Math.random() * legalMoves.length)];
